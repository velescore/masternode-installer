...
#Github	https://github.com/marsmensch, https://gist.github.com/lukechilds, https://github.com/sucremoneda, https://github.com/chaoabunga/mnscripts http://github.com/fxtc
scriptversion="v0.1.1"
release=""
tags="tags/$release"
git="https://github.com/Velescore/Veles-0.16.git"
date="$(date +%y-%m-%d-%s)"
...
curl --silent "https://api.github.com/repos/Velescore/Veles-0.16/releases/latest" | grep -Po '"tag_name": "\K.*?(?=")'
EOF
)"
}
...
function create_sentinel() {
	...
	if [ ! -d ${sent_dir} ]; then
		cd ${data_dir}                                               &>> ${logfile}
		git clone https://github.com/Velescore/sentinel.git sentinel  	&>> ${logfile}
		cd sentinel                                                 &>> ${logfile}
	else
...
ExecStart=${daemon_dir}/${daemon} -daemon -testnet -port=21337 -pid=${data_dir}/${name}/testnet/${name}.pid -conf=${conf_dir}/${name}.conf -datadir=${data_dir}/${name}
...
function print_logo() {
    cd ~/
	wget -q https://i.imgur.com/BxabqzP.png 2>&1
	mv BxabqzP.png veles.png 2>&1
	convert veles.png veles.jpg 2>&1
	rm veles.png
	mv veles.jpg /usr/local/bin/veles.jpg 2>&1
    jp2a -b --colors --width=55 /usr/local/bin/veles.jpg 2>&1
...
if [[ "$testnet" -eq 1  ]]; then
	...
	sudo -u ${user} -- bash -c "${daemon_dir}/${daemon} -testnet -port=21337 -conf=${conf_dir}/${name}.conf -datadir=${data_dir}/${name}"
else
	systemctl daemon-reload
	sleep 30
...
function build_mn() {
    ...
    # compilation starts here
        ...
        if [[ ! -f ${daemon_dir}/${daemon} ]]; then
                echo "COMPILATION FAILED! Please engage with us in Discord https://discord.gg/P528fGg Thank you!"
                exit 1
        fi
}
#WIP
...
#!/bin/bash
latest_release=$(curl --silent "https://api.github.com/repos/Velescore/Veles-0.16/releases/latest" | grep -Po '"tag_name": "\K.*?(?=")')
pause(){ 
...
#	}
	show_menus() {
	clear
	jp2a -b --colors --width=55 /usr/local/bin/veles.jpg
	echo "~~~~~~~~~~~~~~~~~~~~~"
