...
COIN_TGZ='https://github.com/Velescore/Veles-0.16/releases/download/v16/linuxunknown.tar.gz'
...
function create_user() {
  echo -e "Setting up new user for masternode.."
  ...
  if id "$USER" >/dev/null 2>&1; then
    echo "* User exists already, do nothing"                       
     ...
     useradd -m $USER
     su - $USER -c "cd - >/dev/null 2>&1"
     su - $USER -c "mkdir $CONFIGFOLDER >/dev/null 2>&1"	 
     su - $USER -c "touch $CONFIGFOLDER/$CONFIG_FILE >/dev/null 2>&1"
